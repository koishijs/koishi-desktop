...
func (manager *KoiManager) Start(startInstances bool) error {
	...
	if runtime.GOOS == "windows" {
		if startInstances {
			cmd = exec.Command(os.Getenv("COMSPEC"), "/C", "START", "/B", "/MIN", "", "CALL", manager.exe, "run", "daemon") //nolint:gosec
		} else {
			cmd = exec.Command(os.Getenv("COMSPEC"), "/C", "START", "/B", "/MIN", "", "CALL", manager.exe, "--no-start", "run", "daemon") //nolint:gosec
		}
	} else {
		if startInstances {
